...
class Self_Attn(nn.Module):
    ...
    def forward(self,x):
        ...
        out = 0.005*self.gamma*out + x  # I have changed 0.1
            ...
            if no_antialias_up:
                ...
                model += [Upsample(ngf * mult),
                          nn.Conv2d(ngf * mult, int(ngf * mult / 2),
                                    ...
                                    padding=1,  # output_padding=1,
                                    bias=use_bias),nn.ReLU(True),
                          Self_Attn(int(ngf * mult / 2)),
                          norm_layer(int(ngf * mult / 2)),
                          nn.ReLU(True)]

        model += [nn.ReflectionPad2d(3)]
        #model += [cc(ngf),nn.LeakyReLU(0.2,True)]
...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, no_antialias=False):
        ...
        for n in range(1, n_layers):  # gradually increase the number of filters
            ...
            if(no_antialias):
                sequence += [ 
                    nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=kw, stride=1, padding=padw, bias=use_bias), # by me stride 2 to 1
        ...
        sequence += [Self_Attn(ndf * nf_mult_prev, 'relu'),
            nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=kw, stride=1, padding=padw, bias=use_bias),
            norm_layer(ndf * nf_mult),
            nn.LeakyReLU(0.2, True)
        ]
