...
import numpy as np
from criss_cross_attention import CrissCrossAttention as cc
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        for i in range(n_downsampling):  # add downsampling layers
            mult = 2 ** i
            if(no_antialias):
                model += [cc(int(ngf * mult)),nn.ReLU(True),nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, bias=use_bias),
                          norm_layer(ngf * mult * 2),
                          nn.ReLU(True)]
            else:
                model += [cc(int(ngf * mult)),nn.ReLU(True),nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=1, padding=1, bias=use_bias),
                          norm_layer(ngf * mult * 2),
                          nn.ReLU(True),
        ...
        for i in range(n_downsampling):  # add upsampling layers
            ...
            if no_antialias_up:
                model += [cc(int(ngf * mult)),nn.ReLU(True),nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
                ...
                model += [Upsample(ngf * mult),cc(int(ngf * mult)),nn.ReLU(True),
...
class ResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        conv_block += [cc(dim),nn.ReLU(True),nn.Conv2d(dim, dim, kernel_size=3, padding=p, bias=use_bias), norm_layer(dim), nn.ReLU(True)]
        if use_dropout:
...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, no_antialias=False):
        ...
        for n in range(1, n_layers):  # gradually increase the number of filters
            ...
            nf_mult = min(2 ** n, 8)
            if(no_antialias):
                sequence += [ cc(ndf * nf_mult_prev),nn.LeakyReLU(0.2, True),
        ...
        sequence += [cc(ndf * nf_mult),nn.LeakyReLU(0.2, True),
